...
public class ThreadPoolMag {
    private static ThreadPoolMag instance = null;
    ...
    private ThreadPoolMag() {
        this(5);
    }

    private ThreadPoolMag(int num) {
        worker_num = num;
    ...
    public static synchronized ThreadPoolMag getInstance() {
        if (instance == null)
            instance = new ThreadPoolMag();
        return instance;
    }
    ...
    private class WorkThread extends Thread {
        ...
        public void run() {
            while (isRuning) {
                ...
                synchronized (taskQueue) {
                    // 任务队列为空，等待新的任务加入
                    while (isRuning && taskQueue.size() == 0) {
                        try {
                            taskQueue.wait(20);
